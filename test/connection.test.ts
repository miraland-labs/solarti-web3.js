...
describe('Connection', function () {
    ...
    const value = [
      {
        ...
        data: ['', 'base64'],
        executable: false,
        // rentEpoch: Math.pow(2, 64) - 1, // new version feature, MI
        rentEpoch: 0,
        // space: 0,
      },
      {
        ...
        value: [
          {
            account: {
              data: ['', 'base64'],
              executable: false,
              lamports: LAMPORTS_PER_MLN - feeCalculator.lamportsPerSignature,
          ...
          },
          {
            account: {
              ...
              executable: false,
              lamports:
                0.5 * LAMPORTS_PER_MLN - feeCalculator.lamportsPerSignature,
              owner: programId.publicKey.toBase58(),
              rentEpoch: 20,
            },
      ...
      programAccounts.forEach(function (keyedAccount) {
        if (keyedAccount.pubkey.equals(account0.publicKey)) {
          expect(keyedAccount.account.lamports).to.eq(
            LAMPORTS_PER_MLN - feeCalculator.lamportsPerSignature,
          );
        } else {
  ...
  if (process.env.TEST_LIVE) {
    it('stake activation should return activating for new accounts', async () => {
      ...
      const MIN_STAKE_DELEGATION = LAMPORTS_PER_MLN;
  ...
  it('request airdrop', async () => {
    ...
    expect(accountInfo.lamports).to.eq(LAMPORTS_PER_MLN);
    ...
    expect(parsedAccountInfo.lamports).to.eq(LAMPORTS_PER_MLN);
  ...
  if (process.env.TEST_LIVE) {
      ...
      expect(response.value.accounts).to.eql([
        {
          data: ['', 'base64'],
          executable: false,
          lamports: LAMPORTS_PER_MLN - 5000,
          owner: SystemProgram.programId.toBase58(),
          // rentEpoch: Math.pow(2, 64) - 1, // new version feature, MI
          rentEpoch: 0,
          // space: 0,
    ...
    it('multi-instruction transaction', async () => {
      ...
      // accountFrom may have less than LAMPORTS_PER_MLN due to transaction fees
      expect(
    ...
    describe('given an open websocket connection', () => {
      ...
      it('account change notification', async () => {
        ...
        try {
          ...
          connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_MLN);
      ...
      it('logs notification', async () => {
        ...
        try {
          ...
          await connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_MLN);
    ...
    describe('v0 transaction', () => {
      ...
      it('send and confirm', async () => {
        ...
        const transferIxData = encodeData(SYSTEM_INSTRUCTION_LAYOUTS.Transfer, {
          lamports: BigInt(LAMPORTS_PER_MLN),
        });
        addressTableLookups = [
          {
        ...
        expect(transferToAccount?.lamports).to.be.eq(LAMPORTS_PER_MLN);
      });
